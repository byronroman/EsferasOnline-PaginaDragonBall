...
// Script Omitir Introducción y Omitir Outro.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const introEndTime = 105; // 1:45 * 60 (convertido a segundos)
  const outroDuration = 97; // Duración de la outro en segundos

  video.addEventListener('timeupdate', function() {
    // Calcular la duración total del video
    const videoDuration = video.duration;

    // Calcular el tiempo de finalización del capítulo
    const chapterEndTime = 1290; // 21:30 * 60 (convertido a segundos)

    // Calcular el tiempo de inicio de la outro
    const outroStartTime = chapterEndTime - outroDuration;

    // Mostrar u ocultar el botón de "Omitir Introducción"
    ...
    // Mostrar u ocultar el botón de "Omitir Outro"
    if (video.currentTime > outroStartTime && video.currentTime < chapterEndTime) {
  ...
  skipOutroButton.addEventListener('click', function() {
    ...
    const chapterEndTime = 1290; // 21:30 * 60 (convertido a segundos)
    const outroStartTime = chapterEndTime - outroDuration;
    video.currentTime = chapterEndTime;
  });
});