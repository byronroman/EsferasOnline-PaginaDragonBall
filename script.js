...
function reloadSlider() {
  let offset = -active * (100 / items.length);
  ...
  function showSlide(index) {
    slides.forEach((slide, i) => {
      if (i === index) {
        ...
        slide.style.border = '2px solid blue'; // Añade borde azul a la imagen actual
      } else {
        slide.style.transform = 'translateX(-100%)';
        slide.style.border = 'none'; // Elimina el borde de las otras imágenes
      }
    });
  }
});
